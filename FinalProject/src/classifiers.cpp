...
void dldaTrn(double** X, int* y, int N, int d, int* ind, DLDA* dlda)
{
	...
	double** cov_hat_1 = make_2D_matrix(d, d, dinit);
	double** pooled_cov = make_2D_matrix(d, d, dinit);
	double** inv_diagonal = make_2D_matrix(d, d, dinit);
	...
	/*pooled_cov becomes D matrix 162-167 lines*/
    for(i=0; i<d; i++) {
        for(j=0; j<d; j++) {
            if(i!=j)
                pooled_cov[i][j] = 0;
    ...
    invert_matrix(pooled_cov, d, inv_diagonal);

	multiply_A_by_b (inv_diagonal, temp, d, d, (*dlda).a);

	sum_two_matrix(&mu_hat_1, &mu_hat_0, 1, d, 1.00, 1.00, 1.00, &temp);
	multiply_a_by_b ((*dlda).a, temp, d, &temp_b);
	...
	delete_2D_matrix(d, d, cov_hat_1);
	delete_2D_matrix(d, d, pooled_cov);
	delete_2D_matrix(d, d, inv_diagonal);
...
void g13Trn(double** X, int* y, int N, int d, int* ind, G13* g13)
{
    ...
    /*multiplier is our (N_0 + N_1 - 2 - p) / (N_0 + N_1 -2)*/
	multiplier = ((double)(N_0 + N_1 - 2 - d)) / ((double)(N_0 + N_1 -2));

	sum_two_matrix(&mu_hat_1, &mu_hat_0, 1, d, 1.00, -1.00, 1/multiplier, &temp); //(mu_hat_1 - mu_hat_0)*multiplier
	sum_two_matrix(cov_hat_0, cov_hat_1, d, d, (double) (N_0-1), (double) (N_1-1), (double) (N_0+N_1-2), pooled_cov);
	invert_matrix(pooled_cov, d, inv_pooled_cov);
	multiply_A_by_b (inv_pooled_cov, temp, d, d, (*g13).a);

	sum_two_matrix(&mu_hat_1, &mu_hat_0, 1, d, 1.00, 1.00, 1.00, &temp);
	multiply_a_by_b ((*g13).a, temp, d, &temp_b);
...
void qdaTrn(double** X, int* y, int N, int d, int* ind, QDA* qda)
{
	...
	double* mu_hat_0 = new double [d];
	double* mu_hat_1 = new double [d];
	double* temp_b0 = new double [d];
	double* temp_b1 = new double [d];

    double** cov_hat_0 = make_2D_matrix(d, d, dinit);
	double** cov_hat_1 = make_2D_matrix(d, d, dinit);

    double** L_0 = make_2D_matrix(d, d, dinit); //decomposition: Cholesky factor
    double** L_1 = make_2D_matrix(d, d, dinit); //decomposition: Cholesky factor

    double** K_0 = make_2D_matrix(d, d, dinit);
    double** K_1 = make_2D_matrix(d, d, dinit);
	...
	/*cov_hat becomes middle paranthesis 284-291 lines*/
    for(i = 0; i<d; i++) {
        cov_hat_0[i][i] = cov_hat_0[i][i] + a;
        cov_hat_1[i][i] = cov_hat_1[i][i] + a;
    }

    chol_dc(cov_hat_0, d, L_0);
    chol_dc(cov_hat_1, d, L_1); //because every a*I+C is positive-definite matrix

    for(i = 0; i<d; i++) {
        det_0 = det_0 * L_0[i][i] * L_0[i][i];
        det_1 = det_1 * L_1[i][i] * L_1[i][i]; //determinant is equal to square of each diagonal elements of Cholesky factor
    }

	invert_matrix(cov_hat_0, d, K_0); //K_0 is inverted middle paranthesis
	invert_matrix(cov_hat_1, d, K_1); //K_1 is inverted middle paranthesis

	sum_two_matrix(K_1, K_0, d, d, 1.00, -1.00, 2.00, (*qda).a); //qda.a is ready

	multiply_A_by_b (K_0, mu_hat_0, d, d, temp_b0);
	multiply_A_by_b (K_1, mu_hat_1, d, d, temp_b1);
	sum_two_vector(temp_b0, temp_b1, d, 1.00, -1.00, 1.00, (*qda).b); //qda.b is ready


    multiply_a_by_b (temp_b0, mu_hat_0, d, &temp_c0);
    multiply_a_by_b (temp_b1, mu_hat_1, d, &temp_c1);

    (*qda).c = 0.5*(temp_c1 - temp_c0 + log(det_1/det_0)) - log((double)N_1/(double)N_0); //qda.c is ready
    ...
    delete_2D_matrix(d, d, K_0);
    delete_2D_matrix(d, d, K_1);
    delete_2D_matrix(d, d, cov_hat_0);
	delete_2D_matrix(d, d, cov_hat_1);
    delete_2D_matrix(d, d, L_0);
    delete_2D_matrix(d, d, L_1);
	return;
}
...
double qdaTst(double** X, int* y, int N, int d, int* ind, QDA qda)
{
	...
	for (i=0; i<N; i++)
	{
        ...
        multiply_A_by_b (qda.a, temp_X, d, d, temp);
		multiply_a_by_b (temp, temp_X, d, &temp_a);

		multiply_a_by_b (temp_X, qda.b, d, &temp_b);

		y_hat = ((temp_a+temp_b+qda.c) <= 0) ? 1 : 0;
		if (y_hat != y[i])
			temp_error++;
	}
...
void sedcTrn(double** X, int* y, int N, int d, int* ind, SEDC* sedc)
{
	...
	for(i = 0; i<d; i++) {
        z_0 += mu_hat_0[i]; //z_0 is just sum of elements of mu_hat_0
        z_1 += mu_hat_1[i]; //z_1 is just sum of elements of mu_hat_1
	}

	(*sedc).a = z_0 - z_1; // (*sedc).a is ready
	(*sedc).b = (z_0 - z_1) * (-(z_0/d + z_1/d))/2 - log((double)N_1/(double)N_0); //(*sedc).b is ready
...
double sedcTst(double** X, int* y, int N, int d, int* ind, SEDC sedc)
{
	...
	for (i=0; i<N; i++)
	{
        ...
        temp_Z = temp_Z/d; // temp_Z is average
        temp = (sedc.a) * temp_Z;

		y_hat = ((temp+sedc.b) <= 0) ? 1 : 0;
		if (y_hat != y[i])
			temp_error++;
...
void rldaTrn(double** X, int* y, int N, int d, int* ind, RLDA* rlda, int i)
{
	int k, j;
	int N_0 = 0;
	int N_1 = 0;
	...
	/*pooled_cov becomes a matrix, that depends on y_opt (middle paranthesis). 619-625 lines*/
	...
	invert_matrix(pooled_cov, d, inv_pooled_cov);
	multiply_A_by_b (inv_pooled_cov, temp, d, d, (*rlda).a);

	sum_two_matrix(&mu_hat_1, &mu_hat_0, 1, d, 1.00, 1.00, 1.00, &temp);
	multiply_a_by_b ((*rlda).a, temp, d, &temp_b);
